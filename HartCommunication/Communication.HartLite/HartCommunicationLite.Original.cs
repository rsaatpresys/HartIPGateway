...
namespace Communication.HartLite
{
    public class HartCommunicationLite
    {
        ...
        public byte[] SendRaw(byte[] hartFrameDatawithoutPreamble, int preambleLeghtSize = 10)
        {
            ...
            try
            {
                ...
                if (HasCommunicationError())
                {
                    if (ShouldRetry())
                    {
                        SendRawCommnand(hartFrameDatawithoutPreamble);
            ...
            catch (Exception exception)
            {
                ...
                if (ShouldRetry())
                    return SendRaw(hartFrameDatawithoutPreamble);
        ...
        private void SendRawCommnand(byte[] hartFrameDatawithoutPreamble, int preambleLeghtSize = 10)
        {
            ...
            if (SendingCommand != null)
                SendingCommand.BeginInvoke(this, null, null, null);
